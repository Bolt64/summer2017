...
\begin{defn}[$\Omega$-Sprectrum]
  A sprectrum is a $\mathbb{Z}$ indexed sequence of pointed spaces $\calz(n)$ together with
  ...
  then the sprectrum is called an $\Omega$-sprectrum.
\end{defn}

\begin{prop}
  Given a $\Omega$-sprectrum $\calz$, one can define the following functor.
  \begin{align*}
...
In the previous section, we saw that if we are given an $\Omega$-sprectrum, we can construct a
reduced cohomology theory using the sprectrum. Brown's representability theorem is the converse of
the previous theorem, i.e. given a reduced cohomology theory which satisfies the $(\dv)$ axiom, it
can be represented by an $\Omega$-sprectrum, which is unique up to homotopy. This theorem is
...
\begin{thm}[Brown's representability theorem]
  ...
  $\Omega$-sprectrum $\calz$ such that $\redco^{n}$ is naturally isomorphic to
  $\left[\cdot, \calz(n) \right]$. Furthermore, this $\Omega$-sprectrum is unique up to homotopy.
\end{thm}
