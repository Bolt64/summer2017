...
qJ &= \id_{Y}
\end{align*}
We can also see the map $Jq$ is homotopic to $\id_{Z(f)}$ relative to the $Y$ subspace. This means $Z(f)$ is homotopy equivalent to $Y$ and $q$ and $J$ are the homotopy equivalence. Note that $j$ is a closed embedding. We have thus decomposed $f$ into a closed embedding $j$, and a homotopy equivalence $q$.
